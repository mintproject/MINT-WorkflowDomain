...
for crop_name in data:
    data[crop_name]['percent_yield'] = (data[crop_name]['scenario_grain'] - data[crop_name]['base_grain']) / data[crop_name]['base_grain']
    #elasticity
    data[crop_name]['elasticity'] = data[crop_name]['percent_yield'] / data[crop_name]['percent_increase_fertilizer']

# write out as csv file
csv_file = sys.argv[-1:][0]

crops_econ = ['cassava','groundnuts','corn','sesame','sorghum']
default_value = ['0.25','0.25','-99','0.25','-99']
...
with open(csv_file, 'w', newline='') as csvfile:
    ...
    for idx, crop_econ in enumerate(crops_econ):
        ...
        if crop_econ in (name.lower() for name in data):
            ...
            yieldwriter.writerow([tt, default_value[idx]])
