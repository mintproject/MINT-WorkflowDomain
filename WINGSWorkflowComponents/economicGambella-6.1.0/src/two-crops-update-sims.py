...
def _generate_sim_production_cost(pc, sim_production_c1, sim_production_c2):
	i = Path(pc)
	o = Path("simproductioncost_Gam.csv")
...
def _main():
	...
	sorghum_p = float(sys.argv[6]) / 100
	sorghum_c1 = float(sys.argv[7]) / 100
	sorghum_c2 = float(sys.argv[8]) / 100

	a_p = [maize_p,sorghum_p]
	a_c1 = [maize_c1,sorghum_c1]
	a_c2 = [maize_c2,sorghum_c2]
