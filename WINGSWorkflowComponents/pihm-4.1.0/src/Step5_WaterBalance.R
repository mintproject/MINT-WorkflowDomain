...
wb=cbind(P=prcp, 
         ...
         Qd=xl$rivqdown[,oid]/AA) * dt * 1000
twb=time(wb); tid = which(twb <= as.POSIXct('2000-01-02') )
plot(wb[tid,], col=c(4, 3, 4, 2), lty=c(1, 2, 2,2),  
     legend.loc='topright')

dsf=sum((last(xl$eleysurf) -xinit$minit[,4] )* ia * 1 )/AA * 1000
dus=sum((last(xl$eleyunsat) -xinit$minit[,5]) * ia * porosity)/AA * 1000 
dgw=sum((last(xl$eleygw) -xinit$minit[,6] )* ia * porosity)/AA *  1000
rivArea = pr@rivertype[, 'Width'] * pr@river[,'Length']
dst=sum( (as.numeric(last(xl$rivystage)) - xinit$rinit[,2]) * rivArea ) /AA*1000
dst
ss=c(apply(wb, 2,sum), 
